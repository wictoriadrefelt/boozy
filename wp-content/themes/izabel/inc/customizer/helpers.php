 ...
 */
function izabel_register_option( $wp_customize, $option ) {
 ...
 */
function izabel_sort_sections_list( $wp_customize ) {
	foreach ( $wp_customize->sections() as $section_key => $section_object ) {
		if ( false !== strpos( $section_key, 'izabel_' ) && 'izabel_reset_all' !== $section_key && 'izabel_important_links' !== $section_key ) {
			$options[] = $section_key;
		}
	}
...
add_action( 'customize_register', 'izabel_sort_sections_list', 999 );
 ...
 * @since Izabel 1.0
 */
function izabel_section_visibility_options() {
	$options = array(
	...
	return apply_filters( 'izabel_section_visibility_options', $options );
}
 ...
 * @since Izabel 1.0
 */
function izabel_get_pagination_types() {
	$pagination_types = array(
		'default'         => esc_html__( 'Default(Older Posts/Newer Posts)', 'izabel' ),
		'numeric'         => esc_html__( 'Numeric', 'izabel' ),
	);

	return apply_filters( 'izabel_get_pagination_types', $pagination_types );
}
 ...
 * @return post_array
 */
function izabel_generate_post_array( $post_type = 'post' ) {
	$output = array();
