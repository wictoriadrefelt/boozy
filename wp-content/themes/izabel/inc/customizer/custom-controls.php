 ...
 */
function izabel_custom_controls( $wp_customize ) {
	// Custom control for Important Links.
	class Izabel_Important_Links_Control extends WP_Customize_Control {
		...
		public function render_content() {
			...
			$important_links = array(
				'theme_instructions' => array(
					'link'  => esc_url( 'https://catchthemes.com/themes/izabel/#theme-instructions' ),
					'text'  => esc_html__( 'Theme Instructions', 'izabel' ),
					),
				'support' => array(
					'link'  => esc_url( 'https://catchthemes.com/support/' ),
					'text'  => esc_html__( 'Support', 'izabel' ),
					),
				'changelog' => array(
					'link'  => esc_url( 'https://catchthemes.com/themes/izabel/#changelog' ),
					'text'  => esc_html__( 'Changelog', 'izabel' ),
					),
				'facebook' => array(
					'link'  => esc_url( 'https://www.facebook.com/catchthemes/' ),
					'text'  => esc_html__( 'Facebook', 'izabel' ),
					),
				'twitter' => array(
					'link'  => esc_url( 'https://twitter.com/catchthemes/' ),
					'text'  => esc_html__( 'Twitter', 'izabel' ),
					),
				'gplus' => array(
					'link'  => esc_url( 'https://plus.google.com/+Catchthemes/' ),
					'text'  => esc_html__( 'Google+', 'izabel' ),
					),
				'pinterest' => array(
					'link'  => esc_url( 'http://www.pinterest.com/catchthemes/' ),
					'text'  => esc_html__( 'Pinterest', 'izabel' ),
					),
			);
	...
	class Izabel_Multi_Categories_Control extends WP_Customize_Control {
		...
		public function render_content() {
			$dropdown = wp_dropdown_categories(
				array(
					...
					'hide_if_empty'    => false,
					'show_option_all'  => esc_html__( 'All Categories', 'izabel' ),
				)
			);
			...
			echo '<p class="description">' . esc_html__( 'Hold down the Ctrl (windows) / Command (Mac) button to select multiple options.', 'izabel' ) . '</p>';
		}
	}

	// Custom control for any note, use label as output description.
	class Izabel_Note_Control extends WP_Customize_Control {
	...
	class Izabel_Toggle_Control extends WP_Customize_Control {
		public $type = 'light';
...
add_action( 'customize_register', 'izabel_custom_controls', 1 );
