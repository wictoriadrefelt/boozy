	...
	public static function bd_main_ticker_function() {
		...
		<div class="wrap">
			...
			if ( isset( $_REQUEST['bdRestoreDefault'] ) && isset( $_GET['updated'] ) && 'true' == sanitize_text_field( wp_unslash( $_GET['updated'] ) ) ) { //phpcs:ignore
				echo '<div class="updated" ><p>' . esc_html__( 'Ticker setting restored successfully.', 'blog-designer' ).'</p></div>'; //phpcs:ignore
			} elseif ( isset( $_GET['updated'] ) && 'true' == sanitize_text_field( wp_unslash( $_GET['updated'] ) ) ) { //phpcs:ignore
				echo '<div class="updated" ><p>' . esc_html__( 'Ticker settings updated.', 'blog-designer' ) .'</p></div>'; //phpcs:ignore
			}
							...
							<ul class="bd-settings">
								<li>
									...
									<div class="bd-right">
										<span class="fas fa-question-circle bd-tooltips-icon"><span class="bd-tooltips"><?php esc_html_e( 'Enter News Ticker Label', 'blog-designer' ); ?></span></span>
											...
											$news_ticker_label = esc_html__( 'Latest Blog', 'blog-designer' );//phpcs:ignore
											if ( isset( $settings['news_ticker_label'] ) ) {
												$orderby = $settings['news_ticker_label']; //phpcs:ignore
											}
											?>
								...
								<li>
									...
									<div class="bd-right">
										...
										<div class="quantity">
											...
											$posts_per_page = '5'; //phpcs:ignore
											if ( isset( $settings['posts_per_page'] ) ) {
												$orderby = $settings['posts_per_page']; //phpcs:ignore
											}
											?>
								...
								<div class="bd-right">
									...
									if ( isset( $settings['bdp_blog_order_by'] ) ) {
										$orderby = $settings['bdp_blog_order_by']; //phpcs:ignore
									}
									?>
									...
									<div class="blg_order">
										...
										if ( isset( $settings['bdp_blog_order'] ) ) {
											$order = $settings['bdp_blog_order']; //phpcs:ignore
										}
										?>
