...
function bd_load_plugin() {
	...
	if ( '' != $first_review_time && $first_review_time > 0 ) { //phpcs:ignore
	...
	if ( isset( $_GET['blog_designer_promo'] ) && 0 == (int) $_GET['blog_designer_promo'] ) { //phpcs:ignore
		if ( $second_review_time < ( time() - ( 60 * 60 * 24 * 3 ) ) ) {
...
if ( ! function_exists( 'bd_promo' ) ) {
	...
	function bd_promo() {
		...
		echo '<script>jQuery(document).ready( function() {(function($) {$("#blog_designer_promo .blog_designer_promo-close").click(function(){var data;$("#blog_designer_promo").hide();$.post("' . admin_url( '?blog_designer_promo=0' ) . '", data, function(response) {});})})(jQuery)});</script>'; //phpcs:ignore
		echo '
            <style>
