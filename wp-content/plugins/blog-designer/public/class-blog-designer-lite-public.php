...
class Blog_Designer_Lite_Public {
	...
	public function bd_front_stylesheet() {
		...
		wp_enqueue_script( 'ticker', plugins_url( 'js/ticker.min.js', __FILE__ ), array('jquery'));
	...
	public function bd_views() {
		...
		$order     = 'DESC';
		$orderby   = 'date';
		if ( isset( $settings['bdp_blog_order_by'] ) && '' != $settings['bdp_blog_order_by'] ) { //phpcs:ignore
			$orderby = $settings['bdp_blog_order_by'];
		}
		if ( isset( $settings['bdp_blog_order'] ) && isset( $settings['bdp_blog_order_by'] ) && '' != $settings['bdp_blog_order_by'] ) { //phpcs:ignore
		...
		$pagination_type = isset( $settings['pagination_type'] ) && '' != $settings['pagination_type'] ? esc_attr( $settings['pagination_type'] ) : 'paged'; //phpcs:ignore
		if ( $loop->have_posts() ) {
			...
			if ( $max_num_pages > 1 && 'load_more_btn' === $pagination_type &&  'crayon_slider' !== $theme && 'sallet_slider' !== $theme && 'blog-carousel' !== $theme ) {
				...
				if ( '' == $is_loadmore_btn ) { //phpcs:ignore
					...
					$template .= ( isset( $settings['loadmore_button_text'] ) && '' != $settings['loadmore_button_text'] ) ? $settings['loadmore_button_text'] : esc_html__( 'Load More', 'blog-designer' ); //phpcs:ignore
			...
			if ( 'crayon_slider' !== $theme && 'sallet_slider' !== $theme && 'blog-carousel' !== $theme && 'paged' === $pagination_type ) {
				...
				<div class="blog-tickers">
					...
					while ( $loop->have_posts() ) :
						$loop->the_post();
							?>
							<li>
                                <a class="blog-ticker-anchor" href="#"><?php echo get_the_title(); ?></a>
