...
<style type="text/css">
	...
	.bdp_blog_template.sallet_slider .blog_header > div > div > div {
		<?php echo ( '' != $background ) ? 'background: ' . esc_attr( $background ) . ';' : ''; //phpcs:ignore ?>
	}
	.bdp_blog_template.sallet_slider .blog_header > div > div > div::before {
		<?php echo ( '' != $templatecolor ) ? 'background: ' . esc_attr( $templatecolor ) . ' none repeat scroll 0 0;' : ''; //phpcs:ignore ?>
	}
	...
	.bdp_blog_template.media-grid .post-meta .postdate {
		<?php
		if ( '' != $content_fontsize ) { //phpcs:ignore
	...
	/** 12.0 - Crayon Slider Template */
		<?php
		if ( 'design2' == $settings['slider_design_type'] ) { //phpcs:ignore ?>
	...
	<?php
		if ( '' != $grid_hoverback_color ) { //phpcs:ignore
	...
	.blog_template.blog-grid-box .post_title a {
		<?php
		if ( '' != $titlecolor ) { //phpcs:ignore
			echo 'color: ' . esc_attr( $titlecolor ). ';';
		}
		if ( '' != $titlebackcolor ) { //phpcs:ignore
			echo 'background: ' . esc_attr( $titlebackcolor ). ';';
		}
		if ( '' != $template_titlefontsize ) { //phpcs:ignore
	...
	#blog-ticker-style-1 .ticker-style-title {
		<?php
		if ( '' != $template_labeltextcolor ) { //phpcs:ignore
